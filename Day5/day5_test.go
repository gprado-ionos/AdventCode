package main

import "testing"

func Test_lowestSeedLocation(t *testing.T) {
	var seeds = "seeds: 79 14 55 13"

	var seedMapping = []string{"seed-to-soil map:",
		"50 98 2",
		"52 50 48",

		"soil-to-fertilizer map:",
		"0 15 37",
		"37 52 2",
		"39 0 15",

		"fertilizer-to-water map:",
		"49 53 8",
		"0 11 42",
		"42 0 7",
		"57 7 4",

		"water-to-light map:",
		"88 18 7",
		"18 25 70",

		"light-to-temperature map:",
		"45 77 23",
		"81 45 19",
		"68 64 13",

		"temperature-to-humidity map:",
		"0 69 1",
		"1 0 69",

		"humidity-to-location map:",
		"60 56 37",
		"56 93 4"}

	var mapOfAlmanacs map[string][]AlmanacMap = make(map[string][]AlmanacMap)
	baseline := readSeeds(seeds)
	for i := 0; i < len(mapTypes); i++ {
		baseline = buildMapping(seedMapping, mapTypes[i], baseline, mapOfAlmanacs)
	}
	location := findLowestLocation(mapOfAlmanacs)

	if location != 35 {
		t.Errorf("Result was incorrect, got: %d, want: %d.", location, 35)
	}
}

func Test_lowestSeedLocationBySeedRange(t *testing.T) {
	var seeds = "seeds: 79 14 55 13"

	var seedMapping = []string{"seed-to-soil map:",
		"50 98 2",
		"52 50 48",

		"soil-to-fertilizer map:",
		"0 15 37",
		"37 52 2",
		"39 0 15",

		"fertilizer-to-water map:",
		"49 53 8",
		"0 11 42",
		"42 0 7",
		"57 7 4",

		"water-to-light map:",
		"88 18 7",
		"18 25 70",

		"light-to-temperature map:",
		"45 77 23",
		"81 45 19",
		"68 64 13",

		"temperature-to-humidity map:",
		"0 69 1",
		"1 0 69",

		"humidity-to-location map:",
		"60 56 37",
		"56 93 4"}

	var mapOfAlmanacs map[string][]AlmanacMap = make(map[string][]AlmanacMap)
	baseline := readSeedsRange(seeds)
	for i := 0; i < len(mapTypes); i++ {
		baseline = buildMapping(seedMapping, mapTypes[i], baseline, mapOfAlmanacs)
	}
	location := findLowestLocation(mapOfAlmanacs)

	if location != 46 {
		t.Errorf("Result was incorrect, got: %d, want: %d.", location, 46)
	}
}
